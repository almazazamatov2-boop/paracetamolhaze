...
const SEAT_POSITIONS = [
    ...
    { left: 40, top: 15 }, // Seat 5 (Top Mid Left)
    { left: 60, top: 15 }, // Seat 6 (Top Mid Right)
...
const PokerChip = ({ value, index }: { value: number, index: number }) => {
    ...
    return (
        <div className={`w-6 h-6 rounded-full border-2 ${colorClass} shadow-lg flex items-center justify-center relative`} style={{ marginTop: index > 0 ? -18 : 0, zIndex: 50 + index }}>
            <span className="text-[6px] font-black text-white drop-shadow-md">{value}</span>
        </div>
    )
}
  ...
  return (
    <div className="fixed inset-0 bg-[#1a0f0a] overflow-hidden select-none text-white font-sans flex items-center justify-center">
      ...
      <div className="absolute inset-0 opacity-40 pointer-events-none" style={{
          background: 'radial-gradient(circle at 50% 50%, #3d2a1d 0%, #1a0f0a 100%)',
      }} />
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{
          backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.5'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E")`
      }} />

      {/* TOP PANELS */}
      <div className="absolute top-8 left-8 z-50">
          <div className="text-white/40 text-sm font-black uppercase tracking-widest mb-1">NL Hold'em</div>
          <div className="text-4xl font-black italic tracking-tighter">${settings.blind.toFixed(2)} / ${(settings.blind * 2).toFixed(2)}</div>
      </div>

      <div className="absolute top-8 right-8 z-50 flex gap-4">
          {[ {icon: <SettingsIcon />, label: 'Настройки'}, {icon: <Users />, label: 'Лобби'}, {icon: <LogOut />, label: 'Выйти'} ].map((b, i) => (
              <button key={i} onClick={i > 0 ? onBack : undefined} className="w-24 h-24 bg-black/60 border border-white/5 rounded-2xl flex flex-col items-center justify-center hover:bg-white/10 transition-all shadow-2xl group">
      ...
      <div className="relative w-[1200px] aspect-[2/1] rounded-[300px] border-[20px] border-[#2b1e14] shadow-[0_50px_100px_rgba(0,0,0,0.8),inset_0_0_100px_rgba(0,0,0,0.5)] overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-20" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='f'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23f)'/%3E%3C/svg%3E")`, mixBlendMode: 'overlay' }} />
      ...
      <div className="absolute inset-0 pointer-events-none">
          {Array.from({ length: MAX_SEATS }).map((_, i) => {
              ...
              return (
                  <div key={i} className="absolute transition-all duration-700" style={{ left: `${pos.left}%`, top: `${pos.top}%`, transform: 'translate(-50%, -50%)' }}>
                      <div className={`relative flex flex-col items-center pointer-events-auto ${gp?.folded ? 'opacity-30 grayscale' : ''}`}>
                          ...
                          <div className={`w-44 h-56 bg-[#0c0c0c] rounded-3xl border-2 transition-all duration-500 overflow-hidden shadow-2xl ${isTurn ? 'border-orange-500 ring-8 ring-orange-500/20 scale-110 z-50' : 'border-white/10'}`}>
                              <div className="h-[80%] relative bg-black group">
                                  ...
                                  {!p && <div className="absolute inset-0 flex items-center justify-center text-white/5 font-black text-4xl uppercase tracking-tighter">ПУСТО</div>}
                              ...
                              {gp?.isDealer && <div className="absolute top-2 right-2 w-7 h-7 bg-white text-black rounded-full flex items-center justify-center font-black text-xs shadow-xl">D</div>}
                          </div>
                          ...
                          {gp?.bet > 0 && (
                              <div className="absolute -top-32 flex flex-col items-center">
                                  ...
                                  <div className="mt-2 bg-black/90 px-5 py-1.5 rounded-full border border-white/10 text-xs font-black italic shadow-2xl">${gp.bet.toFixed(2)}</div>
                              </div>
                          )}
                      </div>
                  </div>
      ...
      <div className="absolute bottom-10 right-10 z-[100] flex flex-col items-end gap-6">
          <button onClick={() => setShowHandRanks(true)} className="w-16 h-16 bg-black/60 rounded-2xl flex items-center justify-center border border-white/5 hover:bg-white/10 transition-all text-white/20 hover:text-white shadow-2xl">
              <HelpCircle className="w-8 h-8" />
          </button>

          <AnimatePresence>
            {gameState !== 'waiting' && (
              <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className={`w-[600px] bg-black/95 rounded-3xl border border-white/10 p-8 shadow-[0_50px_150px_rgba(0,0,0,1)] transition-all ${isMyTurn ? 'opacity-100 scale-100' : 'opacity-40 pointer-events-none scale-95'}`}>
                  <div className="flex gap-4 mb-8 h-20">
                      <button className="flex-1 bg-red-950/40 border border-red-500/20 rounded-2xl font-black text-2xl italic uppercase hover:bg-red-900/60 transition-all">ФОЛД</button>
                      <button className="flex-1 bg-blue-950/40 border border-blue-500/20 rounded-2xl font-black text-2xl italic uppercase hover:bg-blue-900/60 transition-all flex flex-col items-center justify-center">КОЛЛ <span className="text-xs font-bold text-blue-400 mt-1">$10.00</span></button>
                      <button className="flex-1 bg-green-950/40 border border-green-500/20 rounded-2xl font-black text-2xl italic uppercase hover:bg-green-900/60 transition-all flex flex-col items-center justify-center">РЕЙЗ ДО <span className="text-xs font-bold text-green-400 mt-1">$40.00</span></button>
                  </div>

                  <div className="flex items-center gap-6 mb-8 px-2">
                      <button className="w-12 h-12 rounded-full bg-white/5 hover:bg-white/10 text-2xl font-bold transition-all">-</button>
                      <input type="range" className="flex-1 h-2 bg-white/10 rounded-lg appearance-none accent-orange-500 cursor-pointer" />
                      <button className="w-12 h-12 rounded-full bg-white/5 hover:bg-white/10 text-2xl font-bold transition-all">+</button>
                      <div className="w-32 bg-black/60 border border-white/10 rounded-2xl p-3 text-center text-xl font-black italic text-orange-500">$40.00</div>
                  </div>

                  <div className="grid grid-cols-5 gap-3">
                      {['МИН', '1/2', '2/3', 'ПОТ', 'МАКС'].map(l => <button key={l} className="py-3 bg-white/5 border border-white/5 rounded-xl text-[10px] font-black uppercase text-white/30 hover:text-white hover:bg-white/10 transition-all">{l}</button>)}
                  </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {/* OPTIONS */}
      <div className="absolute bottom-10 left-10 z-50 bg-black/60 backdrop-blur-2xl p-8 rounded-[40px] border border-white/5 flex flex-col gap-5 shadow-2xl">
          {['Фолд на любую ставку', 'Пропускать раздачи', 'Авто-докупка'].map((l, i) => (
              <label key={i} className="flex items-center gap-4 cursor-pointer group">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest text-white/30 group-hover:text-white transition-colors">{l}</span>
              </label>
          ))}
      </div>
      ...
      <AnimatePresence>
        {winnerInfo.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-2xl p-20">
             <div className="max-w-2xl w-full bg-black/40 border border-orange-500/50 p-20 rounded-[60px] text-center shadow-[0_0_150px_rgba(255,165,0,0.2)]">
                <Trophy className="w-24 h-24 text-yellow-500 mx-auto mb-10 animate-bounce" />
                <h2 className="text-7xl font-black italic uppercase tracking-tighter mb-6">ПОБЕДИТЕЛЬ!</h2>
                <div className="text-4xl font-black text-orange-500 uppercase tracking-widest mb-4">{players.find(p => p.id === winnerInfo[0]?.id)?.name || 'КТО-ТО'}</div>
                <div className="text-white/20 text-lg font-black uppercase tracking-[0.5em] mb-16">{winnerInfo[0]?.handName}</div>
                <button className="w-full bg-orange-500 hover:bg-orange-600 py-6 rounded-3xl font-black text-2xl italic uppercase tracking-widest shadow-2xl transition-all active:scale-95">Следующая раздача</button>
             </div>
          </motion.div>
        )}
      </AnimatePresence>
